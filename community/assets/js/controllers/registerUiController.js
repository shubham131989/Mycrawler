...
angular.module('app', ['ui.select'])
    ...
    .controller('registerCtrl', ['$scope', '$localStorage', '$http', '$state', '$sce', '$window', '$stateParams', 'fileUploadService', function($scope, $localStorage, $http, $state, $sce, $window, $stateParams, fileUploadService) {
        ...
        $scope.sendOtp = function(){
            var method = "POST";
            var url = "assets/scripts/sendOtp.php";
        ...
        $scope.register = function (){
            ...
            var data = {user: $scope.user, company: $scope.company, currencyId: $scope.currency.selected.currencyId, address: $scope.address, partnerId: $scope.partnerId};
        ...
        $scope.$on('gmPlacesAutocomplete::placeChanged', function(){
          ...
          if(!$scope.company.companyName){
              $scope.company.companyName = place.name;
          }
          if(place.formatted_phone_number && !$scope.company.contactNo){
              $scope.company.contactNo = place.formatted_phone_number.replace(/\s/g,'');
          }
          if(place.website && !$scope.company.website){
              $scope.company.website = place.website;
          }
...
angular.module('app')
    .service('fileUploadService', function ($http, $q) {
        this.uploadFileToUrl = function (file, uploadUrl, companyId, itemName, description, categoryId, itemId) {
