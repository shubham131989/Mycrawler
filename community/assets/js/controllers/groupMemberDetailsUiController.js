...
angular.module('app', ['ui.select'])
    .controller('groupMemberDetailsCtrl', ['$scope', '$http', '$localStorage', '$state', '$sce', '$compile', '$stateParams', 'fileUploadService', function($scope, $http, $localStorage, $state, $sce, $compile, $stateParams, fileUploadService) {
        ...
        $http({
            method : "GET",
            url : "assets/scripts/getSubscriberDetails.php",
            params: {companyId: $scope.companyId, subscriberId: $scope.subscriberId},
            headers: {'Cache-Control':'no-cache', 'Pragma':'no-cache'},
            cache: false
        }).then(function mySuccess(response) {
            console.log(response);
            $scope.subscriber = response.data.subscriber;
            $scope.parentName.selected = response.data.parent;
            $scope.spouseName.selected = response.data.spouse;
            $scope.education.selected = response.data.education;
            $scope.degree.selected = response.data.degree;
            $scope.maritalStatus.selected = response.data.maritalStatus;
        ...
        $scope.updateGroupMember = function(){
            ...
            var uploadUrl = "assets/scripts/updateSubscriber.php", //Url of webservice/api/server
            ...
            promise.then(function (response) {
                //Item addition successful. Update array objects
                    if(response.flag == 1){
                        window.alert("Group Member successfully updated!");
                        $state.go('app.groupMembers', {});
                    }else{
                        $scope.message = response.message;
                        window.alert("Error in updating Group Member");
